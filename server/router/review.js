...
router.get("/getQuestionData", async (req, res) => {
	try {
		...
		func.forEach((element) => {
			if (element[0] === "2") braFunc[0] = true;
			if (element[0] === "3") braFunc[1] = true;
			if (element[0] === "4") braFunc[2] = true;
			if (element[0] === "6") braFunc[3] = true;
			if (element[0] === "7") braFunc[4] = true;
		});
		...
		winston.debug(util.inspect({ success: true, message: "Recommend complete", braFunc: braFunc, questionData: questionData }, false, null, true));
		return res.json({ success: true, message: "Recommend complete", braFunc: braFunc, questionData: questionData });
	} catch (err) {
		winston.error(err);
...
router.get("/getReviewData", async (req, res) => {
	try {
		...
		res.json({ success: true, message: "리뷰 가져오기 성공", review });
	} catch (err) {
		winston.error(err);
		res.json({ success: false, message: "리뷰 가져오기 실패", err });
	}
});
