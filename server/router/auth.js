    ...
    async (accessToken, refreshToken, profile, done) => {
      ...
      try {
        ...
        if (created) {
          ...
          const result2 = await user.update({ username: userInfo.username, birthday: userInfo.birthday, token: userInfo.token });
    ...
    async (accessToken, refreshToken, profile, done) => {
      ...
      let userInfo = {
        username:
          ...
          profile._json.family_name + profile._json.given_name ||
          "사용자",
        email: profile._json.email,
