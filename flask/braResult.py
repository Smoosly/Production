...
def result():
	try:
		...
		df_result = pd.DataFrame(index=range(0,0), columns=['PK_ID', 'PK_ITEM', 'OLD_KEY', 'SIZE', 'COLOR'])
		df_fix = pd.DataFrame(index=range(0,0), columns = ['PK_ID', 'COMPLETE', 'NUM', 'SIZE', 'PK_ITEM_1', 'OLD_KEY_1', 'SIZE_1', 'PK_SIZE_1', 'PK_ITEM_2', 'OLD_KEY_2', 'SIZE_2', 'PK_SIZE_2', 'PK_ITEM_3', 'OLD_KEY_3', 'SIZE_3', 'PK_SIZE_3', 'PK_ITEM_4', 'OLD_KEY_4', 'SIZE_4', 'PK_SIZE_4'])
		...
		for idx, row in df.iterrows():
			...
			for decision in decisions:
				...
				fixArray.extend([ row['PK_ITEM_{}'.format(decision)], row['OLD_KEY_{}'.format(decision)], sizes,  row['PK_ITEM_{}'.format(decision)][:2]+generalSize ])
		...
		df_result = pd.merge(left = df_result, right = df_brAll, how = 'left', on = ['PK_ITEM', 'OLD_KEY'])
		...
		for idx, row in df_result.iterrows(): 
			...
			cur.execute(sql_insert, (row.PK_ID, row.PK_ITEM, row.OLD_KEY, row.SIZE, row.COLOR, row.LINK, row.PRICE))
			cur.execute(sql_insert_2, (row.PK_ID, row.PK_ITEM, row.OLD_KEY, row.COLOR, 0,0,0,0))
		...
		for idx, row in df_fix.iterrows():
			...
			cur.execute(sql_insert, (row.PK_ID, row.NUM, row.SIZE, row.PK_ITEM_1, row.OLD_KEY_1, row.SIZE_1, row.PK_SIZE_1, row.PK_ITEM_2, row.OLD_KEY_2, row.SIZE_2, row.PK_SIZE_2,\
				row.PK_ITEM_3, row.OLD_KEY_3, row.SIZE_3, row.PK_SIZE_3,row.PK_ITEM_4, row.OLD_KEY_4, row.SIZE_4, row.PK_SIZE_4))
