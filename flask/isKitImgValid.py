...
def getVolume(mINNER_LEN, mINNER_WIDTH_LC, mOUTER_LEN, mOUTER_WIDTH_LC, mHEIGHT):
    ...
    if len(height) != 0:
        ...
        mVolume = (
            (mINNER_WIDTH_LC + mOUTER_WIDTH_LC)
            / 2
            * 0.25
            * mHEIGHT
            * math.pi
...
def save_as_graph(arr, filename, dir, ratio, rratio, xDiff, yDiff):
    ...
    finalImg = cv2.resize(
        finalImg, dsize=(1000, int(1000 * rratio)), interpolation=cv2.INTER_LANCZOS4
    )
    finalImg = cv2.resize(
        finalImg,
        dsize=(0, 0),
    ...
    finalImg = cv2.resize(
        finalImg, dsize=(0, 0), fx=0.2, fy=0.2, interpolation=cv2.INTER_LANCZOS4
    )
...
def measure(filename, type, dir=0):
    ...
    approx = np.squeeze(
        np.array(cv2.approxPolyDP(cont, cv2.arcLength(cont, True) * 0.01, True)), 1
    )
    xMin = approx[:, 0].min()
    xMax = approx[:, 0].max()
    yMin = approx[:, 1].min()
    ...
    dirTrue = [
        ...
        (0, cv2.ROTATE_90_COUNTERCLOCKWISE),
    ]  # forward, 90_COUNTERLOCKWISE, 90_CLOCKWISE, converted
    ...
    for idx, rectangle in enumerate(data):
        ...
        if idx == 2:
            grayImg = morph.copy()[
                int(yMin + imgDropOffset[1][idx]) : int(yMax - imgDropOffset[1][idx]),
                int(xMin + imgDropOffset[0][idx]) : int(
                    xMax - imgDropOffset[0][idx] * 2
                ),
            ]
        ...
        if imgDirection[idx] == 0:
            ...
            save_as_graph(
                lsImg, filename, dir, ratio, aspectRatio, xMax - xMin, yMax - yMin
            )
        ...
        if kitType == 0:  # Breast kit
            ...
            if leftImgPath != "":
                ...
                if isinstance(measureResult, dict):
                    ...
                    mInnerLenL, mLowerLenL, lowerShapeL, mOuterLenL = measure(
                        leftImgPath, type=0
                    )
                whereBt = 0
                (
                ...
                mVolumeL = getVolume(
                    mInnerLenL, mInnerWidthLcL, mOuterLenL, mOuterWidthLcL, mHeightLcL
                )
            ...
            if rightImgPath != "":
                ...
                if isinstance(measureResult, dict):
                    ...
                    mInnerLenR, mLowerLenR, lowerShapeR, mOuterLenR = measure(
                        rightImgPath, type=0, dir=1
                    )
                ...
                mVolumeR = getVolume(
                    mInnerLenR, mInnerWidthLcR, mOuterLenR, mOuterWidthLcR, mHeightLcR
                )
