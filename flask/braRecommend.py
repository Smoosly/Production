        ...
        try:
                ...
                _, _, _, _, _, whereBt, _, _, _, _, _, _, _, _, _, mHeightLcL, _, _, _, _, mHeightLcR, _, _, _, _, _, _, _,\
                        mUnderBust, mUpperBust, _, _, braSizeUb, braSizeCup, _, _, _, _, _, _,\
                                _, _, _, _, _, _, wPressure, _, wPP, wBrEffect, wBrEffectMost, _, _, wDesignConcept, _, _, wPriceMax, wImportant, _, _, _ = result_breastTest[0]
                ...
                # Get BR_CH
                cur.execute(sql_brCh)
                result_brCh = cur.fetchall()
                df_brCh = pd.DataFrame(result_brCh)
                sql_column = 'select column_name from information_schema.columns where table_name = %s order by ordinal_position'
                cur.execute(sql_column, ("BR_CH"))
                result_column = cur.fetchall()
                columns = []
                for i in result_column:
                        columns.append(i[0])
                        
                df_brCh.columns = columns

                # Get BR_NUM
                cur.execute(sql_brNum)
                result_brNum = cur.fetchall()
                df_brNum = pd.DataFrame(result_brNum)
                sql_column = 'select column_name from information_schema.columns where table_name = %s order by ordinal_position'
                cur.execute(sql_column, ("BR_NUM"))
                result_column = cur.fetchall()
                columns = []
                for i in result_column:
                        columns.append(i[0])
                        
                df_brNum.columns = columns
                ...
                df['SCORE'] = df.apply(lambda x: x.BREAST_FIT_SCORE + x.EFFECT_FIT_SCORE * effectWeight, axis = 1)
                ...
                cur.execute(sql_recomBra, (pkId, num, decision, Size, wBrEffect, 
                        pkItem1, oldKey1, size1, None, breastScore1, effectScore1, 
                        pkItem2, oldKey2, size2, None, breastScore2, effectScore2,
                        pkItem3, oldKey3, size3, None, breastScore3, effectScore3,
                        pkItem4, oldKey4, size4, None, breastScore4, effectScore4, 
                        pkItem5, oldKey5, size5, None, breastScore5, effectScore5,
                        pkItem6, oldKey6, size6, None, breastScore6, effectScore6, 
                        pkItem7, oldKey7, size7, None, breastScore7, effectScore7, 
                        pkItem8, oldKey8, size8, None, breastScore8, effectScore8, 
                        pkItem9, oldKey9, size9, None, breastScore9, effectScore9, 
                        pkItem10, oldKey10, size10, None, breastScore10, effectScore10))
                db.commit()
                log.info(f"{pkId}'s BOTH Bra Recommend Success!")
